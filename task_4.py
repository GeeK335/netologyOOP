...
def average_all_students(course_name: str, *students):
	combined_grades = []
	for i in range(len(students)):
		combined_grades += students[i][course_name]
	average_rating = sum(combined_grades) / len(combined_grades)
	return average_rating


average_student = ('Средняя оценка за домашние задания по всем студентам в рамках '
                   'конкретного курса:')
print(average_student, average_all_students('Git', evelina_sokolova.grades,
                                            sergey_makarov.grades))
